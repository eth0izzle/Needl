...
class Job(object):
    ...
    * a quantity of `time units` defined by `interval`
    ...
    def __repr__(self):
        ...
        if hasattr(self.job_func, '__module__') and hasattr(self.job_func, '__name__'):
            job_func_name = '%s->%s' % (self.job_func.__module__, self.job_func.__name__)
        else:
        ...
        runinterval = '%s %s' % (self.interval, unit) if self.latest is None else '%(interval)s %(unit)s to %(latest)s %(unit)s' % dict(interval=self.interval, latest=self.latest, unit=unit)
...
def clear(tag=None):
    """Deletes scheduled jobs on the default schedule marked with the given
...
def cancel_job(job):
    """Delete a scheduled job on the default schedule
